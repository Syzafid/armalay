...
- **Posisi 3D Model Pas di Sisi Kiri Screen:** Objek 3D GLB artefak diletakkan pas di bagian kiri layar (`position="-0.55 -0.05 -1.8"`) sehingga model 3D tampil 100% utuh tanpa terpotong di pinggir layar.
- **Console-Style D-Pad Controller (Kanan Bawah):** Disediakan tombol kontrol bergaya konsol game di pojok kanan bawah (Panah `▲`, `▼`, `◄`, `►`, Reset `↺`, serta `🔍+` & `🔍-`) untuk memutar dan memperbesar/memperkecil model 3D secara interaktif.
- **Smart Auto-Hide Controller:** Tombol D-Pad Controller secara otomatis **menghilang saat panel Informasi/Kuis dibuka**, dan **muncul kembali saat panel ditutup**, menjaga antarmuka tetap rapi dan tidak saling tumpang tindih.
